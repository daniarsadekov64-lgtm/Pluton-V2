...
local function fixSectionAfterRefresh() end
local library = Library 


do
    local flagsProxy = {}
    setmetatable(flagsProxy, {
        __index = function(_, key)
            if Toggles and Toggles[key] ~= nil then
        ...
        __newindex = function(_, key, val)
            if Toggles and Toggles[key] then
                pcall(function() Toggles[key]:SetValue(val) end)
        ...
        __pairs = function()
            local t = {}
            if Toggles then for k,v in pairs(Toggles) do t[k] = v.Value end end
            if Options then for k,v in pairs(Options) do t[k] = v.Value end end
            return pairs(t)
        end,
    })
    library.Flags = flagsProxy
...
local Settings = {
    ...
    Misc = {
        ...
        pcldCustom = {
            ...
            color2 = Color3.fromRGB(180, 80, 255),
...
local _playerListCacheAt = 0
function BlobmanBetaFeature.GetPlayerList()
    local now = tick()
    
    if (now - _playerListCacheAt) < 4 and #_playerListCache > 0 then
    ...
    for i = 1, #players do
        ...
        if plr ~= LocalPlayer then
            ...
            if not isProtectedPlayer(plr.Name) then
                list[#list + 1] = (plr.DisplayName or plr.Name) .. " @" .. plr.Name
            end
        end
    end
    _playerListCache = list
    _playerListCacheAt = now
    return list
end
Players.PlayerAdded:Connect(function() _playerListCacheAt = 0 end)
Players.PlayerRemoving:Connect(function() _playerListCacheAt = 0 end)
...
local function GetSnowballPlayerList()
    ...
    if BlobmanBetaFeature and BlobmanBetaFeature.GetPlayerList then
        return BlobmanBetaFeature.GetPlayerList()
    end
    local list = {}
    local players = Players:GetPlayers()
    for i = 1, #players do
...
task.spawn(function()
    ...
    local function applyTitleAndTopBar()
        local sg = Library.ScreenGui
        if not sg then return end
        local scheme = Library.Scheme or {}
        local bg = scheme.MainColor or scheme.BackgroundColor or Color3.fromRGB(22, 22, 24)
        ...
        for _, desc in ipairs(sg:GetDescendants()) do
            ...
            if className == "ImageLabel" or className == "ImageButton" then
                ...
                local img = tostring(desc.Image or "")
                if n:find("icon", 1, true) or n:find("logo", 1, true)
                    or img:find("123697909505352", 1, true) or img:find("17872750744", 1, true) then
                    desc.Visible = false
            ...
            elseif className == "TextLabel" or className == "TextButton" then
                local n = (desc.Name or ""):lower()
                if n:find("title", 1, true) then
                    local t = tostring(desc.Text or "")
                    if (desc.TextSize or 0) >= 14
                        and not t:lower():find("settings", 1, true)
                        and not t:lower():find("config", 1, true) then
                        if t == "" or t:lower():find("pluton", 1, true) or n:find("title", 1, true) then
                            desc.Text = "Pluton V2"
                            desc.Visible = true
            ...
            elseif className == "Frame" or className == "CanvasGroup" then
                local n = (desc.Name or ""):lower()
                if n:find("topbar", 1, true) or n:find("titlebar", 1, true)
                    or n:find("header", 1, true) or n == "title" or n:find("top_bar", 1, true) then
                    desc.BackgroundTransparency = 0
                    desc.BackgroundColor3 = bg
                end
            end
        end
        appliedOnce = true
    ...
    for _ = 1, 6 do
        pcall(applyTitleAndTopBar)
        if appliedOnce then break end
        task.wait(0.2)
    end
    
    task.delay(1.0, function() pcall(applyTitleAndTopBar) end)
    task.delay(2.5, function() pcall(applyTitleAndTopBar) end)
end)
...
task.defer(function()
    pcall(function()
        ...
        if Library.SetCornerRadius then
            Library:SetCornerRadius(14)
        end
        if Library.Corners then
    ...
    local function showTabNames()
        local sg = Library.ScreenGui
        if not sg then return end
        local tabNames = {
            Main = true, Target = true, Anti = true, Misc = true,
            Telekinesis = true, Grab = true, Loop = true, Aura = true,
            Player = true, Visuals = true, ["UI Settings"] = true,
        }
        for _, desc in ipairs(sg:GetDescendants()) do
    ...
    showTabNames()
    task.delay(0.5, showTabNames)
    task.delay(1.5, showTabNames)
    ...
    pcall(function()
        ...
        for _, d in ipairs(sg:GetDescendants()) do
            if d:IsA("ImageLabel") or d:IsA("ImageButton") then
                ...
                local p = d.Parent and (d.Parent.Name or ""):lower() or ""
                if (n:find("icon", 1, true) or p:find("title", 1, true) or p:find("header", 1, true)) then
                    local sz = d.AbsoluteSize
                    if sz.X > 0 and sz.X <= 48 and sz.Y <= 48 then
                        
                        if not (p:find("tab", 1, true) or p:find("sidebar", 1, true) or p:find("nav", 1, true)) then
...
do
    ...
    pcall(function()
        if Library.AddDraggableLabel then
            wmLabel = Library:AddDraggableLabel("Pluton V2  |  60 FPS  |  0 ms")
        elseif Library.AddWatermark then
            wmLabel = Library:AddWatermark("Pluton V2  |  60 FPS  |  0 ms")
        end
    end)
    
    if not wmLabel then
        local sg = Instance.new("ScreenGui")
        sg.Name = "PlutonWatermark"
        ...
        lbl.Name = "WM"
        lbl.Size = UDim2.new(0, 420, 0, 22)
        lbl.Position = UDim2.new(0, 12, 0, 8)
        lbl.BackgroundColor3 = Color3.fromRGB(12, 12, 14)
        lbl.BackgroundTransparency = 0.25
        lbl.BorderSizePixel = 0
        lbl.Text = "Pluton V2  |  60 FPS  |  0 ms"
        ...
        UserInputService.InputChanged:Connect(function(input)
            if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
                ...
                lbl.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y)
            end
        end)
        wmLabel = lbl
    end

    local function setWmText(t)
        if typeof(wmLabel) == "Instance" and wmLabel:IsA("TextLabel") then
            wmLabel.Text = t
        elseif typeof(wmLabel) == "table" then
            pcall(function()
                if wmLabel.SetText then wmLabel:SetText(t)
                elseif wmLabel.Set then wmLabel:Set(t)
                elseif wmLabel.Text ~= nil then wmLabel.Text = t
                end
            end)
        elseif typeof(wmLabel) == "Instance" then
            pcall(function()
                if wmLabel:IsA("TextLabel") or wmLabel:IsA("TextButton") then
                    wmLabel.Text = t
                else
                    local inner = wmLabel:FindFirstChildWhichIsA("TextLabel", true)
                    if inner then inner.Text = t end
                end
            end)
        end
    end
    ...
    RunService.Heartbeat:Connect(function()
        ...
        if now - _wmLastPingAt < 0.5 then return end
        _wmLastPingAt = now
        pcall(function()
            local stats = game:GetService("Stats")
            local item = stats.Network.ServerStatsItem["Data Ping"]
            if item then
                _wmPing = math.floor(item:GetValue())
            end
        end)
        local nick = LocalPlayer.DisplayName or LocalPlayer.Name
        local clock = os.date("%H:%M:%S")
        setWmText(("Pluton V2  |  %s  |  %s  |  %d FPS  |  %d ms"):format(nick, clock, math.floor(FPS), _wmPing))
    end)
end
...
local Tabs = {
    ...
    Telekinesis = Window:AddTab("Telekinesis", "sparkles"),
    Grab = Window:AddTab("Grab", "hand"),
    Loop = Window:AddTab("Loop", "repeat"),
    Aura = Window:AddTab("Aura", "circle"),
...
local BlobmanTab = Tabs.Target
local AntiTab = Tabs.Anti
local MiscTab = Tabs.Misc
local TelekinesisTab = Tabs.Telekinesis
local GrabTab = Tabs.Grab
local LoopTab = Tabs.Loop
local AuraTab = Tabs.Aura
local PlayerTab = Tabs.Player


local function _makeCompatGroupbox(tab, name, side)
    side = side or "Left"
    local gb
    if side:lower() == "right" then
        gb = tab:AddRightGroupbox(name)
    else
        gb = tab:AddLeftGroupbox(name)
    end

    local proxy = {}
    local self = proxy

    function proxy:Toggle(opts)
        opts = opts or {}
        local flag = opts.Flag or opts.Text or ("Toggle_" .. tostring(math.random(1,999999)))
    ...
    function proxy:Slider(opts)
        opts = opts or {}
        local flag = opts.Flag or opts.Text or ("Slider_" .. tostring(math.random(1,999999)))
        local minv = opts.Minimum or opts.Min or 0
        local maxv = opts.Maximum or opts.Max or 100
        local def = opts.Default or minv
        local suffix = opts.ValueName or opts.Suffix or ""
        gb:AddSlider(flag, {
            Text = opts.Text or flag,
            Default = def,
            Min = minv,
            Max = maxv,
            Rounding = opts.Rounding or (type(def) == "number" and (def % 1 ~= 0 and 1 or 0) or 0),
    ...
    function proxy:Dropdown(opts)
        opts = opts or {}
        local flag = opts.Flag or opts.Text or ("Dropdown_" .. tostring(math.random(1,999999)))
        ...
        local lastListHash = nil
        local function listHash(t)
            if type(t) ~= "table" then return tostring(t) end
            local n = #t
            if n == 0 then return "0" end
            return tostring(n) .. "|" .. tostring(t[1]) .. "|" .. tostring(t[n])
        end
        lastListHash = listHash(list)
        local obj = {
            Refresh = function(_, newOpts)
                newOpts = newOpts or {}
                local newList = newOpts.List or newOpts.Values or list
                local h = listHash(newList)
                if h == lastListHash then return end
                lastListHash = h
                list = newList
                ...
                task.defer(function()
                    if Options[flag] then
                        pcall(function()
                            Options[flag]:SetValues(newList)
                        end)
                    end
                end)
            end,
            SetValue = function(_, v)
                task.defer(function()
                    if Options[flag] then pcall(function() Options[flag]:SetValue(v) end) end
                end)
            end,
        }
        return obj
    ...
    function proxy:Keybind(opts)
        opts = opts or {}
        local flag = opts.Flag or opts.Text or ("Keybind_" .. tostring(math.random(1,999999)))
        local mode = opts.Mode or "Toggle"
        local default = opts.Default or "Unknown"
        ...
        local safeCb = userCb
        if userCb and tostring(mode):lower() ~= "hold" then
            safeCb = function(...)
                local args = table.pack(...)
                task.defer(function()
                    pcall(userCb, table.unpack(args, 1, args.n))
                end)
            end
        end
        local label = gb:AddLabel(opts.Text or flag)
        label:AddKeyPicker(flag, {
            Default = default,
            Text = opts.Text or flag,
            Mode = mode,
            Callback = safeCb,
    ...
    function proxy:Button(opts)
        opts = opts or {}
        gb:AddButton({
            Text = opts.Text or "Button",
            Func = opts.Callback or opts.Func or function() end,
    ...
    function proxy:Label(opts)
        if type(opts) == "string" then
            gb:AddLabel(opts)
        else
            opts = opts or {}
            gb:AddLabel(opts.Text or "")
        end
        return self
    end
...
local function patchTab(tab)
    if not tab or tab._endorisPatched then return tab end
    tab._endorisPatched = true
    local oldAddLeft = tab.AddLeftGroupbox
    local oldAddRight = tab.AddRightGroupbox
    function tab:Section(opts)
        opts = opts or {}
        local name = opts.Text or opts.Name or "Section"
        local side = opts.Side or "Left"
        return _makeCompatGroupbox(self, name, side)
    end
    return tab
end
...
function Window:AddTab(name, icon)
    local t = _origAddTab(self, name, icon or "box")
    return patchTab(t)
end

function Window:Tab(opts)
    opts = opts or {}
    local name = opts.Text or opts.Name or "Tab"
    return self:AddTab(name, "box")
end
...
ownershipKickSec:Button({Text = "Refresh Players", Callback = function()
    pcall(function()
        ownershipKickCombo:Refresh({Text = "Select Target", List = BlobmanBetaFeature.GetPlayerList()})
    end)
    fixSectionAfterRefresh(ownershipKickCombo)
end})
ownershipKickCombo = ownershipKickSec:Dropdown({
    Text = "Select Target",
    Flag = "OwnershipKickTarget",
    List = BlobmanBetaFeature.GetPlayerList(),
    Callback = function(value)
        if type(value) == "table" then
            value = value[1] or value.Name or ""
        end
        local name = tostring(value or "")
        if name == "" or name == "nil" then return end
        local at = string.match(name, "@(.+)$")
        ownershipKickTarget = at or name
    end,
})

local selectedKickMode = "Ownership Kick (80+ fps)"
ownershipKickSec:Dropdown({
    Text = "Ownership Mode",
    Flag = "OwnershipKickMode",
    List = {
...
ownershipKickSec:Toggle({Text = "Enable Ownership Kick", Flag = "OwnershipKickEnable", Default = false, Callback = function(Value)
    if not Value then
        ownershipCleanup()
        return
    end
    ownershipCleanup()
    ownershipKickActive = true
    Settings.Loop.ownershipKickActive = true

    local targetName = ownershipKickTarget
    if not targetName or targetName == "" then
        warn("[OK] Select target first")
        ownershipCleanup()
        return
    end
    ...
    ownershipKickTask = task.spawn(function()
        ...
        if mode == "Ownership Kick (80+ fps)" then
            ...
            if not myRoot then ownershipCleanup() return end
            ...
            local fpsConnection = RunService.RenderStepped:Connect(function(dt)
                if dt > 0 then currentFPS = 1 / dt end
            end)
            ...
            while ownershipKickActive do
                ...
                local tHum = tChar:FindFirstChildOfClass("Humanoid")
                if not tRoot or not tHum then RunService.Heartbeat:Wait() continue end
                ...
                if not dragging then
                    ...
                    pcall(function()
                        local n = 2
                        if currentFPS < 155 then n = 4
                        elseif currentFPS <= 200 then n = 3 end
                        for _ = 1, n do
        ...
        elseif mode == "Ownership Kick fast" then
            while ownershipKickActive do
                ...
                if not myRoot or not isAlive(target) then
                    RunService.Heartbeat:Wait()
                    continue
                end
                ...
                if tRoot and tHum then
                    myRoot.CFrame = tRoot.CFrame * CFrame.new(0, -5, -8)
                    tHum.PlatformStand = true
                    pcall(function()
                        for _ = 1, 3 do
                            SetNetOwner:FireServer(tRoot, tRoot.CFrame * CFrame.new(0, 40, 0))
                        end
                        DestroyGrabLine:FireServer(tRoot)
                    end)
                    tRoot.AssemblyLinearVelocity = Vector3.new(0, 250, 0)
                end
                RunService.Heartbeat:Wait()
            end
        ...
        elseif mode == "Ownership Kick For Exploiter" then
            while ownershipKickActive do
                ...
                if tRoot and tHum then
                    ...
                    tHum.PlatformStand = true
                    tHum.Sit = false
                    for _, limbName in ipairs({"Left Leg", "Right Leg", "Left Arm", "Right Arm", "Head", "Torso", "UpperTorso", "LowerTorso"}) do
                    ...
                    pcall(function()
                        for _ = 1, 5 do
                            SetNetOwner:FireServer(tRoot, CFrame.new(0, 1e6, 0))
                        end
                        DestroyGrabLine:FireServer(tRoot)
                    end)
                end
            ...
            local myRoot = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
            local savedPos = myRoot and myRoot.CFrame or CFrame.new()
            local dragging = false
            local grabStartTime = 0
            local checkStartTime = 0
            local bodyPos, bodyGyro = nil, nil
            ...
            while ownershipKickActive do
                ...
                if not dragging then
                    myRoot.CFrame = tRoot.CFrame * CFrame.new(0, 0, 3)
                    if grabStartTime == 0 then grabStartTime = tick() end
                    if tick() - grabStartTime > 0.06 then
                        dragging = true
                        checkStartTime = tick()
                        createBodies(tRoot, (savedPos * CFrame.new(5, 25, 4)).Position)
                    end
                else
                    myRoot.CFrame = savedPos
                    local lockPos = savedPos * CFrame.new(5, 25, 4)
                    if bodyPos and bodyPos.Parent then
                        bodyPos.Position = lockPos.Position
                        if bodyGyro then bodyGyro.CFrame = lockPos end
                    else
                    ...
                    if checkStartTime > 0 and tick() - checkStartTime > 0.2 then
                        if (tRoot.Position - lockPos.Position).Magnitude > 15 then
                            dragging = false
                            grabStartTime = 0
                            checkStartTime = 0
                            cleanupBodies()
...
ownershipKickSec:Slider({Text = "Grab Kick Height", Flag = "GrabKickHeight", Minimum = 5, Maximum = 80, Default = 20, ValueName = "studs", Callback = function(v)
    Settings.Grab.grabKickHeight = v
end})
ownershipKickSec:Slider({Text = "Grab Kick Tilt", Flag = "GrabKickTilt", Minimum = 0, Maximum = 90, Default = 45, ValueName = "deg", Callback = function(v)
    Settings.Grab.grabKickTilt = v
end})
ownershipKickSec:Toggle({Text = "Kick (Grab + Hold)", Flag = "GrabKickEnable", Default = false, Callback = function(Value)
    grabKickActive = Value
    if not Value then
        if grabKickTask then
            pcall(function() task.cancel(grabKickTask) end)
            grabKickTask = nil
        end
        
        pcall(function()
            if _G.PlutonStopXocuKick then _G.PlutonStopXocuKick() end
        end)
        return
    end

    local targetName = ownershipKickTarget
    if (not targetName or targetName == "") and Settings.BlobmanBeta and Settings.BlobmanBeta.selectedTarget then
        targetName = Settings.BlobmanBeta.selectedTarget
        ownershipKickTarget = targetName
    end
    if not targetName or targetName == "" then
        warn("[Kick] Select target first")
        pcall(function()
            Library:Notify({ Title = "Error",
                Content = "Select target first!", Description = "Select target first!", Time = 3 })
        end)
        grabKickActive = false
        return
    end

    
    pcall(function()
        if _G.PlutonStartXocuKick then _G.PlutonStartXocuKick() end
    end)

    grabKickTask = task.spawn(function()
        local GE = ReplicatedStorage:FindFirstChild("GrabEvents")
        if not GE then
            GE = ReplicatedStorage:WaitForChild("GrabEvents", 5)
        end
        if not GE then
            warn("[GrabKick] GrabEvents missing")
            grabKickActive = false
            return
        end
        ...
        local homePos = myRoot.Position
        local homeLook = myRoot.CFrame.LookVector
        
        if homePos.Y > 80 or homePos.Y < -20 then
            homePos = Vector3.new(homePos.X, math.clamp(homePos.Y, 0, 30), homePos.Z)
        end
        local function homeCF()
            return CFrame.new(homePos, homePos + Vector3.new(homeLook.X, 0, homeLook.Z))
        end
        ...
        local lastRemote = 0
        local lastRegrab = 0
        local REMOTE_DELAY = 0.03
        local REGRAB_COOLDOWN = 0.4
        ...
        while grabKickActive do
            ...
            local seat = myChar:FindFirstChildOfClass("Humanoid") and myChar:FindFirstChildOfClass("Humanoid").SeatPart
            if seat and seat.Parent and seat.Parent.Name == "CreatureBlobman" and tRoot then
            ...
            if tRoot and tHum and tHum.Health > 0 then
                local kickH = math.clamp((Settings.Grab and Settings.Grab.grabKickHeight) or customKickHeight or 20, 5, 80)
                local kickTilt = math.rad((Settings.Grab and Settings.Grab.grabKickTilt) or 45)
                
                local lockPos = homeCF() * CFrame.new(0, kickH, 0) * CFrame.Angles(kickTilt, 0, 0)

                
                local escapeDist = (tRoot.Position - lockPos.Position).Magnitude
                local targetOnGround = tRoot.Position.Y < (homePos.Y + kickH * 0.35)
                if dragging and (escapeDist > 12 or targetOnGround) and (now - lastRegrab) > REGRAB_COOLDOWN then
                    dragging = false
                    grabStartTime = 0
                    lastRegrab = now
                end
                ...
                if not dragging then
                    ...
                    local elapsed = now - grabStartTime
                    if elapsed < 0.2 then
                        
                        local claimPos = Vector3.new(tRoot.Position.X, math.clamp(tRoot.Position.Y, homePos.Y - 2, homePos.Y + 8), tRoot.Position.Z)
                        myRoot.CFrame = CFrame.new(claimPos)
                        ...
                        pcall(function()
                            ...
                            tRoot.AssemblyLinearVelocity = Vector3.zero
                            if SetNetOwner then
                                SetNetOwner:FireServer(tRoot, myRoot.CFrame)
                            end
                    ...
                    pcall(function()
                        ...
                        tHum.PlatformStand = true
                        tHum.Sit = true
                        if now - lastRemote >= REMOTE_DELAY then
                            lastRemote = now
                            if SetNetOwner then
                                SetNetOwner:FireServer(tRoot, lockPos)
                            end
                            ...
                            if CreateGrabLine then
                                CreateGrabLine:FireServer(tRoot, Vector3.zero, lockPos.Position, false)
                            end
                        end
                    end)
                end
        ...
        pcall(function()
            local root = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
            if root then
                root.CFrame = CFrame.new(homePos, homePos + Vector3.new(homeLook.X, 0, homeLook.Z))
                root.AssemblyLinearVelocity = Vector3.zero
            end
        end)
        grabKickActive = false
...
do
    local antiSec = BlobmanTab:Section({Text = "Anti-AntiKick"})
    local antiMode = "CLICK"
    local antiActive = false
    local antiTask = nil
    local removeAntiActive = false

    local function getSelectedTargetPlayer()
        local name = ownershipKickTarget
        if (not name or name == "") and Settings.BlobmanBeta.selectedTarget then
            name = Settings.BlobmanBeta.selectedTarget
        end
        if not name or name == "" then return nil end
        
        local at = string.find(name, "@")
        if at then name = string.sub(name, at + 1) end
        return Players:FindFirstChild(name)
    end

    antiSec:Dropdown({
        Text = "Anti-AntiKick Mode",
        Flag = "AntiAntiKickMode",
        List = {"CLICK", "BYPASS"},
        Default = "CLICK",
        Callback = function(v)
            antiMode = v
    ...
    antiSec:Toggle({
        Text = "Enable Anti-AntiKick",
        Flag = "EnableAntiAntiKick",
        Default = false,
        Callback = function(Value)
            if antiTask then
            ...
            local StickyEvent = PE and PE:FindFirstChild("StickyPartEvent")
            ...
            local function hasOwner(part)
                local o = part:FindFirstChild("PartOwner")
                return o and o.Value == LocalPlayer.Name
            end

            antiTask = task.spawn(function()
                while antiActive do
                    task.wait()
                    local TargetPLR = getSelectedTargetPlayer()
                    ...
                    if antiMode == "CLICK" then
                        for _, v in pairs(TargetInv:GetChildren()) do
                            ...
                            if StickyPart then
                                ...
                                if not hasOwner(StickyPart) then
                                    sno(StickyPart)
                                end
                            end
                        end
                        ...
                        for _, v in pairs(TargetInv:GetChildren()) do
                            ...
                            local mag = (FirePlayerPart.Position - StickyPart.Position).Magnitude
                            if myFirePart and StickyWeld.Part1 == myFirePart and mag > 7 then
                                continue
                            elseif not hasOwner(StickyPart) then
                                sno(StickyPart)
                            else
                                if StickyEvent then
                                    pcall(function()
                                        StickyEvent:FireServer(StickyPart, FirePlayerPart, CFrame.new(0, -10, 5))
                                    end)
                                end
                            end
                        end
    ...
    antiSec:Toggle({
        ...
        Callback = function(Value)
            ...
            task.spawn(function()
                ...
                while removeAntiActive do
                    local target = getSelectedTargetPlayer()
                    if target then
                        local spawned = workspace:FindFirstChild(target.Name .. "SpawnedInToys")
                        if spawned then
                            for _, name in ipairs({"NinjaKunai", "NinjaShuriken", "AntiKick", "ToolPickaxe", "ToolPencil", "ToolDiggingForkRusty", "ToolCleaver"}) do
    ...
    -- Pallet Ragdoll (working Endoris style)
local prSec = BlobmanTab:Section({Text = "Pallet Ragdoll"})
local palletRagdollTask = nil

prSec:Toggle({Text = "Ragdoll Target", Flag = "PalletRagdollInvis", Default = false, Callback = function(v)
    Settings.Loop.palletRagdollActive = v
    if not v then
        if palletRagdollTask then
            pcall(function() task.cancel(palletRagdollTask) end)
            palletRagdollTask = nil
        end
        task.spawn(function()
            local toysFolder = workspace:FindFirstChild(LocalPlayer.Name .. "SpawnedInToys")
            if toysFolder then
                for _, name in ipairs({"InstrumentDrumSnare", "FoodBread", "PalletLightBrown", "PalletForRagdoll"}) do
                    local toy = toysFolder:FindFirstChild(name)
                    if toy then
                        pcall(function()
                            if DeleteToyRE then DeleteToyRE:FireServer(toy)
                            elseif ReplicatedStorage:FindFirstChild("MenuToys") then
    ...
    if targetName and string.find(targetName, "@") then
        targetName = string.sub(targetName, (string.find(targetName, "@") or 0) + 1)
    end
    if not targetName or targetName == "" then
        warn("[Ragdoll] Select target")
        Settings.Loop.palletRagdollActive = false
        pcall(function()
            Library:Notify({ Title = "Ragdoll", Content = "Select target first!", Time = 3 })
        end)
        return
    end
    ...
    palletRagdollTask = task.spawn(function()
        ...
        local setNE = GE:WaitForChild("SetNetworkOwner", 5)
        local destroyGL = GE:WaitForChild("DestroyGrabLine", 5)
        local createGL = GE:FindFirstChild("CreateGrabLine")
        local SpawnRF = SpawnToyRF or (ReplicatedStorage:FindFirstChild("MenuToys") and ReplicatedStorage.MenuToys:FindFirstChild("SpawnToyRemoteFunction"))
        local DestroyRE = DeleteToyRE or (ReplicatedStorage:FindFirstChild("MenuToys") and ReplicatedStorage.MenuToys:FindFirstChild("DestroyToy"))

        local function checkAlive(t)
            if not t or t == LocalPlayer then return false end
            if isProtectedPlayer and isProtectedPlayer(t.Name) then return false end
        ...
        local function claim(part)
            ...
            if claimFrame % 3 == 0 then
                pcall(function() if setNE then setNE:FireServer(part, part.CFrame) end end)
            elseif claimFrame % 3 == 1 then
                pcall(function()
                    if createGL then createGL:FireServer(part, Vector3.zero, part.Position, false) end
                end)
            else
                pcall(function() if destroyGL then destroyGL:FireServer(part) end end)
            end
        ...
        for _, toyName in ipairs(toyNames) do
            pcall(function()
                if SpawnRF then SpawnRF:InvokeServer(toyName, skyPos, Vector3.zero) end
            end)
            for _ = 1, 100 do
                local inv = workspace:FindFirstChild(LocalPlayer.Name .. "SpawnedInToys")
        ...
        if not drum or not mainPart then
            warn("[Ragdoll] No toy available")
            Settings.Loop.palletRagdollActive = false
            return
        end
        ...
        while Settings.Loop.palletRagdollActive do
            ...
            if not target or not checkAlive(target) then
                task.wait(0.2)
                continue
            end
            ...
            local tRoot = tChar and tChar:FindFirstChild("HumanoidRootPart")
            local tHum = tChar and tChar:FindFirstChildOfClass("Humanoid")
            if not tHead or not tRoot or not tHum or tHum.Health <= 0 then
                task.wait(0.1)
                continue
            end
        ...
        if drum and drum.Parent then
            pcall(function()
                if DestroyRE then DestroyRE:FireServer(drum) end
            end)
            if drum.Parent then pcall(function() drum:Destroy() end) end
        end
        palletRagdollTask = nil
    end)
end})
...
do
    local xocuKicksSec = BlobmanTab:Section({Text = "Kicks"})

    local function getKickTargetName()
        if ownershipKickTarget and ownershipKickTarget ~= "" then
            return ownershipKickTarget
        end
        if Settings.BlobmanBeta and Settings.BlobmanBeta.selectedTarget and Settings.BlobmanBeta.selectedTarget ~= "" then
            return Settings.BlobmanBeta.selectedTarget
        end
        return nil
    end
    ...
    local function sno(part)
        pcall(function()
            if setNetworkOwnerEvent and part and part.Parent then
                setNetworkOwnerEvent:FireServer(part, part.CFrame)
            end
        end)
    end

    local function cleanupXocuKick(targetPlayer)
        ...
        if align then
            if align.Attachment1 then pcall(function() align.Attachment1:Destroy() end) end
            pcall(function() align:Destroy() end)
        end
        if rot then pcall(function() rot:Destroy() end) end
        if att0 then pcall(function() att0:Destroy() end) end
        
        local att1 = workspace.Terrain:FindFirstChild("KickAtt1")
        if att1 then pcall(function() att1:Destroy() end) end
        pcall(function()
            if destroyGrabLineEvent then destroyGrabLineEvent:FireServer(tH) end
        end)
    end

    local function ensureKickAlign(tHRP)
        if tHRP:FindFirstChild("KickAlign") then return end
    ...
    local function stopXocuKick()
        oatsKickActive = false
        if oatsKickTask then
            pcall(function() task.cancel(oatsKickTask) end)
            oatsKickTask = nil
        end
        local name = getKickTargetName()
        local tp = name and Players:FindFirstChild(name)
        cleanupXocuKick(tp)
        pcall(function()
            if Options and Options.OatsKick and Options.OatsKick.SetValue then
                
            end
            if Toggles and Toggles.OatsKick and Toggles.OatsKick.SetValue then
                Toggles.OatsKick:SetValue(false)
            end
        end)
    end

    local function startXocuKick()
        if oatsKickActive then return end
        local targetName = getKickTargetName()
        if not targetName or targetName == "" then
            warn("[Kick] Select target first")
            return
        end
        ...
        oatsKickTask = task.spawn(function()
                ...
                while oatsKickActive do
                    RunService.Heartbeat:Wait()
                    if not oatsKickActive then break end

                    local currentTargetName = getKickTargetName() or targetName
                    local targetPlayer = currentTargetName and Players:FindFirstChild(currentTargetName)
                ...
                cleanupXocuKick(Players:FindFirstChild(targetName))
                oatsKickActive = false
                oatsKickTask = nil
            end)
    end

    _G.PlutonStartXocuKick = startXocuKick
    _G.PlutonStopXocuKick = stopXocuKick
...
do
    ...
    local kickSoundId = "rbxassetid://4590657391" 
    local leaveSoundId = "rbxassetid://4590657391"
    local notifiedKick = {} 
    ...
    local pendingKick = {}
    local PENDING_WINDOW = 12

    
    local function notifyKickConfirmed(displayName, username, force)
        if not username or username == "" then return end
        if notifiedKick[username] and (tick() - notifiedKick[username]) < 8 then
            return
        end
        if not force then
            
            local still = Players:FindFirstChild(username)
            if still then
                return
            end
        end
        notifiedKick[username] = tick()
        pendingKick[username] = nil
        playSound(kickSoundId, 1)
        notify("Pluton V2", string.format("%s (%s) has been kicked", displayName or username, username), 6)
    end

    local function markPossibleKick(displayName, username)
        if not username or username == "" then return end
        if notifiedKick[username] and (tick() - notifiedKick[username]) < 8 then
            return
        end
        local prev = pendingKick[username]
        if prev and (tick() - prev.at) < 1.5 then
            return
        end
        pendingKick[username] = { displayName = displayName or username, at = tick() }

        
        task.spawn(function()
            for _ = 1, 6 do
                task.wait(0.5)
                local p = pendingKick[username]
                if not p then return end
                if not Players:FindFirstChild(username) then
                    notifyKickConfirmed(p.displayName or displayName or username, username, true)
                    return
                end
            end
            
            if pendingKick[username] and Players:FindFirstChild(username) then
                pendingKick[username] = nil
            end
        end)
    end
    ...
    Workspace.ChildAdded:Connect(function(obj)
        if obj.Name ~= "BlackHoleKick" and obj.Name ~= "BlackHoleDetected" then return end
        task.wait(0.05)
        local pos
        if obj:IsA("BasePart") then
            pos = obj.Position
        elseif obj:IsA("Model") then
            local pp = obj.PrimaryPart or obj:FindFirstChild("Hole") or obj:FindFirstChildWhichIsA("BasePart")
            if pp then pos = pp.Position end
        end
        if not pos then return end
        local plr, dist = getClosestPlayer(pos)
        if plr and dist < 80 then
            
            markPossibleKick(plr.DisplayName or plr.Name, plr.Name)
        end
    end)
    ...
    RunService.Heartbeat:Connect(function()
        for _, plr in ipairs(Players:GetPlayers()) do
            ...
            if y > 5000 or vel > 2000 then
                local last = lastHighCheck[plr.UserId]
                if not last or (tick() - last) > 4 then
                    lastHighCheck[plr.UserId] = tick()
    ...
    local function isCurrentTarget(player)
        ...
        if ownershipKickTarget and ownershipKickTarget == name then return true end
        if Settings.BlobmanBeta and Settings.BlobmanBeta.selectedTarget == name then return true end
        return false
    end

    Players.PlayerRemoving:Connect(function(player)
        local name = player.Name
        local disp = player.DisplayName or player.Name
        local pending = pendingKick[name]
        if pending and (tick() - pending.at) < PENDING_WINDOW then
            
            notifyKickConfirmed(pending.displayName or disp, name, true)
        elseif notifiedKick[name] and (tick() - notifiedKick[name]) < 5 then
            
        elseif isCurrentTarget(player) then
            playSound(leaveSoundId, 1)
            notify("Target Left", string.format("%s (@%s) left the game", disp, name), 5)
        end
        pendingKick[name] = nil
        if ownershipKickTarget == name then ownershipKickTarget = nil end
        if Settings.BlobmanBeta and Settings.BlobmanBeta.selectedTarget == name then
...
local function zoomSet(on)
    if on and not zoomActive then
        zoomActive = true
        zoomOrigFOV = (Settings.Misc and Settings.Misc.FOVValue) or Workspace.CurrentCamera.FieldOfView or 70
    ...
    elseif not on and zoomActive then
        zoomActive = false
        pcall(function()
...
miscCamSec:Keybind({Text = "Zoom", Flag = "ZoomBind", Mode = "Hold", Default = "C", Callback = function(held)
    if type(held) == "boolean" then
        zoomSet(held)
    else
        zoomSet(not zoomActive)
    end
end})
...
do
    local partBreakerConn = nil
    local function getMyGrabbedParts()
        local parts = {}
        local grabParts = Workspace:FindFirstChild("GrabParts")
        if not grabParts then return parts end
        local myRoot = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        for _, gp in ipairs(grabParts:GetChildren()) do
            if gp.Name == "GrabPart" and gp:IsA("BasePart") then
                ...
                if weld then
                    local target = nil
                    if weld.Part1 and weld.Part1 ~= gp then target = weld.Part1
                    elseif weld.Part0 and weld.Part0 ~= gp then target = weld.Part0 end
                    if target and target:IsA("BasePart") then
                        
                        if not myRoot or (gp.Position - myRoot.Position).Magnitude < 80 then
    ...
    local function collectParts(part)
        local list = {}
        if not part or not part.Parent then return list end
    ...
    local ownerTick = 0
    local function breakPartCollision(part)
        if not part or not part.Parent then return end
        local parts = collectParts(part)
        local myCF = Utility.GetPlayerCFrame and Utility.GetPlayerCFrame()
        if not myCF then
            local root = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
            myCF = root and root.CFrame
        end
        ownerTick = ownerTick + 1
        for i, p in ipairs(parts) do
            pcall(function()
                
                if myCF and (ownerTick % 2 == 0 or i == 1) then
                    if setNetworkOwnerEvent then
                        setNetworkOwnerEvent:FireServer(p, myCF)
                    elseif FTAP and FTAP.SetNetworkOwner then
                        FTAP.SetNetworkOwner(p, myCF)
                    end
                end
                p.CanCollide = false
                p.CanTouch = false
    ...
    local function startPartBreaker()
        if partBreakerConn then return end
        local last = 0
        partBreakerConn = RunService.Heartbeat:Connect(function()
            if not Settings.Misc.partBreaker then return end
            local now = tick()
            if now - last < 0.03 then return end
            last = now
            for _, part in ipairs(getMyGrabbedParts()) do
                breakPartCollision(part)
            end
        end)
    end

    local function stopPartBreaker()
        if partBreakerConn then
            partBreakerConn:Disconnect()
            partBreakerConn = nil
        end
    end

    miscOtherSec:Toggle({Text = "Part Breaker", Flag = "PartBreaker", Default = false, Callback = function(v)
        Settings.Misc.partBreaker = v
        if v then startPartBreaker() else stopPartBreaker() end
    end})
end
...
miscOtherSec:Toggle({Text = "Highlight Objects", Flag = "HighlightObjects", Default = false, Callback = function(state)
    ...
    State.highlightObjectsConn = RunService.Heartbeat:Connect(function()
        ...
        local now = tick()
        if now - _hlLast < 0.2 then return end
        _hlLast = now
        highlightBodies()
    end)
end})
...
do
    local bindSec = MiscTab:Section({Text = "Feature Binds", Side = "Left"})

    local function obsidianNotify(msg)
        msg = tostring(msg or "")
    ...
    local function getToggle(flag)
        return (Toggles and Toggles[flag]) or (Library.Toggles and Library.Toggles[flag])
    end

    local states = {}
    local lastFire = {}

    local function setToggle(flag, on)
        local t = getToggle(flag)
        if t then
            pcall(function()
                if t.SetValue then t:SetValue(on)
                elseif t.SetState then t:SetState(on)
                else t.Value = on end
            end)
        end
    end
    ...
    local handlers = {
        KickAura = function(on)
            Settings.BlobmanBeta.kickAuraActive = on
            setToggle("KickAura", on)
        end,
        OwnershipKickEnable = function(on) setToggle("OwnershipKickEnable", on) end,
        GrabKickEnable = function(on) setToggle("GrabKickEnable", on) end,
        OatsKick = function(on) setToggle("OatsKick", on) end,
        SkyAura = function(on) setToggle("SkyAura", on) end,
        FlingAura = function(on) setToggle("FlingAura", on) end,
        ClickAura = function(on) setToggle("ClickAura", on) end,
        PalletRagdollInvis = function(on) setToggle("PalletRagdollInvis", on) end,
        TelekinesisEnabled = function(on)
            if Settings.Telekinesis then Settings.Telekinesis.Enabled = on end
            setToggle("TelekinesisEnabled", on)
        end,
    }

    local labels = {
        KickAura = "Kick Aura",
        OwnershipKickEnable = "Ownership Kick",
        GrabKickEnable = "Grab Kick",
        OatsKick = "Hold Kick",
        SkyAura = "Sky Aura",
        FlingAura = "Fling Aura",
        ClickAura = "Click Aura",
        PalletRagdollInvis = "Pallet Ragdoll",
        TelekinesisEnabled = "Telekinesis",
    }

    
    local bindKeys = {
        KickAura = nil,
        OwnershipKickEnable = nil,
        GrabKickEnable = nil,
        OatsKick = nil,
        SkyAura = nil,
        FlingAura = nil,
        ClickAura = nil,
        PalletRagdollInvis = nil,
        TelekinesisEnabled = nil,
    }

    local keyNames = {"None","Q","E","R","T","Y","U","F","G","H","J","K","L","Z","X","C","V","B","N","M"}
    local keyEnums = { None = nil }
    for _, n in ipairs(keyNames) do
        if n ~= "None" then keyEnums[n] = Enum.KeyCode[n] end
    end

    local function fireBind(flag)
        local now = tick()
        if lastFire[flag] and (now - lastFire[flag]) < 0.35 then return end
        lastFire[flag] = now
        local on = not states[flag]
        states[flag] = on
        local h = handlers[flag]
        if h then
            task.defer(function() pcall(h, on) end)
        end
        obsidianNotify((labels[flag] or flag) .. (on and " ON" or " OFF"))
    end

    local orderedFlags = {
        "KickAura", "OwnershipKickEnable", "GrabKickEnable", "OatsKick",
        "SkyAura", "FlingAura", "ClickAura", "PalletRagdollInvis", "TelekinesisEnabled",
    }

    for _, flag in ipairs(orderedFlags) do
        local label = labels[flag] or flag
        bindSec:Dropdown({
            Text = label,
            Flag = "BindKey_" .. flag,
            List = keyNames,
            Default = "None",
            Callback = function(v)
                local name = tostring(v or "None")
                if name == "None" or name == "" then
                    bindKeys[flag] = nil
                elseif keyEnums[name] then
                    bindKeys[flag] = keyEnums[name]
                end
            end,
        })
    end

    bindSec:Label("All binds default to None — assign a key")

    UserInputService.InputBegan:Connect(function(input, gp)
        if gp then return end
        if input.UserInputType ~= Enum.UserInputType.Keyboard then return end
        if UserInputService:GetFocusedTextBox() then return end
        for flag, key in pairs(bindKeys) do
...
local _waterSplashLast = 0
RunService.Heartbeat:Connect(function()
    if not Settings.Misc.waterSplash then return end
    local now = tick()
    if now - _waterSplashLast < 0.05 then return end
...
do
    ...
    local function getKeyFromFlag(flag)
        ...
        local ok, opt = pcall(function() return Options and Options[flag] end)
        if ok and opt then
            if type(opt.Value) == "string" and opt.Value ~= "" and opt.Value ~= "None" and opt.Value ~= "Unknown" then
                return opt.Value
            end
            if type(opt.Value) == "table" then
                local k = opt.Value[1] or opt.Value.Key or opt.Value.key
                if k and tostring(k) ~= "" and tostring(k) ~= "None" and tostring(k) ~= "Unknown" then
                    return tostring(k)
                end
            end
            if opt.DisplayValue and tostring(opt.DisplayValue) ~= "" then
                return tostring(opt.DisplayValue)
            end
        end
        local ok2, data = pcall(function() return library.Flags and library.Flags[flag] end)
    ...
    local function isKeybindEnabled(flag, mode)
        ...
        if mode == "Hold" then
            if keybindHeldStates[flag] == true then return true end
        else
            if keybindToggleStates[flag] == true then return true end
        end
        ...
        if ok and opt then
            local ok2, st = pcall(function()
                ...
                return opt.Value == true or opt.Toggled == true
            end)
            if ok2 and st then return true end
        end
        return false
    end
        ...
        local function rebuildKeyMap()
            ...
            for _, def in ipairs(allKeybindDefs) do
                ...
                if keyName == "" then
                    ...
                    if ok and data and type(data) == "table" then
                        keyName = data.Key or data.key or ""
                    end
                end
                if keyName ~= "" and keyName ~= "None" and keyName ~= "Unknown" then
    ...
    local function rebuildKeybindList()
        ...
        if Toggles then
            for flag, toggle in pairs(Toggles) do
                if type(toggle) == "table" and toggle.Value == true and not ignoreFlags[flag] then
                    local name = toggle.Text or flag
                    if type(name) == "string" and name ~= "" and not seen[name] then
                        local lower = name:lower()
                        if lower ~= "active modules" and lower ~= "keybind list" and not lower:find("menu toggle") then
                            seen[name] = true
        ...
        for _, def in ipairs(allKeybindDefs) do
            if isKeybindEnabled(def.flag, def.mode or "Toggle") then
                local name = def.text
        ...
        local scheme = Library.Scheme or {}
        local sig = tostring(scheme.BackgroundColor) .. tostring(scheme.FontColor) .. tostring(scheme.AccentColor) .. tostring(scheme.MainColor) .. "|"
        for _, m in ipairs(activeModules) do
            sig = sig .. m.name .. (m.key or "") .. "|"
        end
        ...
        local fontColor = scheme.FontColor or Color3.new(1, 1, 1)
        local mainColor = scheme.MainColor or Color3.fromRGB(25, 25, 25)
        local outlineColor = scheme.OutlineColor or Color3.fromRGB(40, 40, 40)
        local bgColor = scheme.BackgroundColor or Color3.fromRGB(15, 15, 15)
        local accentColor = scheme.AccentColor or Color3.fromRGB(125, 85, 255)

        
        if keybindListMain then
            keybindListMain.BackgroundColor3 = bgColor
            local stroke = keybindListMain:FindFirstChildOfClass("UIStroke")
            if stroke then stroke.Color = outlineColor end
            local title = keybindListMain:FindFirstChild("Title")
            if title then title.TextColor3 = fontColor end
            local sep = keybindListMain:FindFirstChild("Separator")
            if sep then sep.BackgroundColor3 = outlineColor end
        end

        for i, m in ipairs(activeModules) do
            local row = Instance.new("Frame")
            row.Name = "ModuleEntry"
            row.Size = UDim2.new(1, 0, 0, 22)
            row.BackgroundColor3 = mainColor
            ...
            if m.key and m.key ~= "" and m.key ~= "Unknown" and m.key ~= "None" then
                nameLbl.Size = UDim2.new(1, -70, 1, 0)
                local keyLbl = Instance.new("TextLabel")
                keyLbl.Name = "Key"
                keyLbl.Size = UDim2.new(0, 56, 1, 0)
                keyLbl.Position = UDim2.new(1, -60, 0, 0)
                keyLbl.BackgroundTransparency = 1
                keyLbl.Text = "[" .. tostring(m.key) .. "]"
        ...
        if #activeModules == 0 then
            ...
            empty.Text = "No active modules"
            empty.TextColor3 = fontColor
            empty.TextTransparency = 0.45
            empty.Font = Enum.Font.Code
            empty.TextSize = 12
            empty.TextXAlignment = Enum.TextXAlignment.Center
            empty.LayoutOrder = 0
            empty.Parent = keybindListContainer
        end

        lastBindCount = #activeModules
        local contentH = 34 + math.max(#activeModules, 1) * 27 + 12
        if #activeModules == 0 then contentH = 60 end
        
        local maxH = 520
        pcall(function()
            local cam = workspace.CurrentCamera
            if cam then
                maxH = math.floor(cam.ViewportSize.Y * 0.7)
            end
        end)
        maxH = math.clamp(maxH, 200, 700)
        if keybindListMain then
            keybindListMain.Size = UDim2.new(0, 260, 0, math.min(contentH, maxH))
        end
        if keybindListContainer and keybindListContainer:IsA("ScrollingFrame") then
            
            local canvasH = math.max(#activeModules, 1) * 27 + 8
            if #activeModules == 0 then canvasH = 28 end
            keybindListContainer.CanvasSize = UDim2.new(0, 0, 0, canvasH)
        end
    end

    miscConfigSec:Toggle({Text = "Active Modules", Flag = "KeybindListToggle", Default = false, Callback = function(v)
        if v then
            keybindListGui = Instance.new("ScreenGui")
            keybindListGui.Name = "ActiveModulesGui"
            ...
            main.Name = "ActiveModulesMain"
            main.Size = UDim2.new(0, 240, 0, 56)
            main.Position = UDim2.new(0.5, -120, 0, 100)
            main.BackgroundColor3 = scheme.BackgroundColor or Color3.fromRGB(15, 15, 15)
            ...
            local stroke = Instance.new("UIStroke")
            stroke.Color = scheme.OutlineColor or Color3.fromRGB(40, 40, 40)
            stroke.Thickness = 1
            ...
            title.TextColor3 = scheme.FontColor or Color3.new(1, 1, 1)
            ...
            sep.Position = UDim2.new(0, 8, 0, 28)
            sep.BackgroundColor3 = scheme.OutlineColor or Color3.fromRGB(40, 40, 40)
            ...
            container.ScrollBarImageColor3 = Color3.fromRGB(180, 180, 180)
            ...
            keybindListConn = RunService.Heartbeat:Connect(function(dt)
                ...
                if keybindListMain and not keybindListMain.Visible then return end
                kbUpdateTimer = kbUpdateTimer + dt
                if kbUpdateTimer < 1.0 then return end
                kbUpdateTimer = 0
                pcall(rebuildKeybindList)
            end)
        else
...
do
    ...
    local infZoomConn1, infZoomConn2, infZoomLoop = nil, nil, nil
    ...
    local function startInfZoomLoop()
        if infZoomLoop then return end
        infZoomLoop = RunService.RenderStepped:Connect(function()
    ...
    local function stopInfZoom()
        ...
        if infZoomLoop then infZoomLoop:Disconnect() infZoomLoop = nil end
        infZoomValue = 0
    end
...
local loopGrabKickAllSec = LoopTab:Section({Text = "Grab Kick All", Side = "Left"})
local grabKickAllActive = false
local grabKickAllTask = nil
local grabKickAllHeight = "Spawn"

loopGrabKickAllSec:Dropdown({
    Text = "Destroy Height",
    Flag = "GrabKickAllHeight",
    List = {"Spawn", "Heaven"},
    Default = "Spawn",
    Callback = function(v)
        grabKickAllHeight = v
...
loopGrabKickAllSec:Toggle({Text = "Grab Kick All", Flag = "GrabKickAll", Default = false, Callback = function(Value)
    grabKickAllActive = Value
    if not Value then
        if grabKickAllTask then
            pcall(function() task.cancel(grabKickAllTask) end)
            grabKickAllTask = nil
        end
        return
    end

    grabKickAllTask = task.spawn(function()
        local GrabEvents = ReplicatedStorage:FindFirstChild("GrabEvents") or ReplicatedStorage:WaitForChild("GrabEvents", 5)
        if not GrabEvents then
            grabKickAllActive = false
            return
        end
        ...
        local function startLineLag()
            ...
            task.spawn(function()
                ...
                while lineLagEnabled and grabKickAllActive do
        ...
        while grabKickAllActive do
            local height = (grabKickAllHeight == "Heaven") and 1e9 or 35

            startLineLag()
            task.wait(1)

            if not grabKickAllActive then break end
...
miscOtherSec:Toggle({Text = "PCLD Visual (use Visuals tab)", Flag = "PCLDVisual", Default = false, Callback = function(value)
    Settings.Misc.pcldVisual = value
    if Settings.Misc.pcldCustom then Settings.Misc.pcldCustom.enabled = value end
    if _G.PlutonPCLDSetEnabled then _G.PlutonPCLDSetEnabled(value) end
end})
...
do
    ...
    function TelekinesisFeature.runAura()
        if TelekinesisFeature.auraConn then
            local st = coroutine.status(TelekinesisFeature.auraConn)
            if st == "dead" then
                TelekinesisFeature.auraConn = nil
            else
                return
...
task.spawn(function()
    ...
    local VisualsState = {
        jumpCircles = false,
        jumpCircleColor = Color3.fromRGB(0, 200, 255),
        jumpCircleSize = 6,
        ...
        chinaHatParts = {},
        chinaHatConn = nil,
        chinaHatColor = Color3.fromRGB(0, 255, 200),
        chinaHatAccent = Color3.fromRGB(255, 255, 255),
        chinaHatTransparency = 0.25,
        chinaHatSize = 1,
        ...
        trails = false,
        trailObjects = {},
        trailColor = Color3.fromRGB(255, 100, 255),
        trailFadeColor = Color3.fromRGB(100, 50, 255),
        trailLifetime = 0.5,
        skybox = false,
        originalSky = nil,
        skyboxId = "rbxassetid://159454299",
        skyboxColor = Color3.fromRGB(255, 255, 255),
        customSkyParts = {},
        shaders = false,
        shaderEffects = {},
        ...
        ccContrast = 0.15,
        ccSaturation = 0.2,
        ccTint = Color3.fromRGB(255, 245, 255),
    ...
    local SKYBOX_PRESETS = {
        ...
        ["Pink Galaxy"] = "rbxassetid://271042516",
        ["Blue Space"] = "rbxassetid://149397684",
        ["Night"] = "rbxassetid://12064107",
        ["Cloudy"] = "rbxassetid://150939027",
        ["Sunset"] = "rbxassetid://401664839",
        ["Purple Void"] = "rbxassetid://644547296",
    }
    ...
    local function destroyChinaHat()
        for _, p in ipairs(VisualsState.chinaHatParts) do
            pcall(function() if p and p.Parent then p:Destroy() end end)
        end
        VisualsState.chinaHatParts = {}
        if VisualsState.chinaHatConn then
            VisualsState.chinaHatConn:Disconnect()
            VisualsState.chinaHatConn = nil
        end
        local char = getChar()
        if char then
            local old = char:FindFirstChild("Hat")
    ...
    local function createChinaHat()
        ...
        hat.Transparency = VisualsState.chinaHatTransparency
        hat.Color = VisualsState.chinaHatColor
        ...
        mesh.Scale = Vector3.new(2.4, 1.6, 2.4) * VisualsState.chinaHatSize
        mesh.Parent = hat
        ...
        hat.CFrame = head.CFrame * CFrame.new(0, 1.1 * VisualsState.chinaHatSize, 0)
        hat.Parent = char
        table.insert(VisualsState.chinaHatParts, hat)
        table.insert(VisualsState.chinaHatParts, mesh)
        table.insert(VisualsState.chinaHatParts, weld)

        
        VisualsState.chinaHatConn = RunService.Heartbeat:Connect(function()
            if not VisualsState.chinaHat then return end
            local h = hat
            if not h or not h.Parent then return end
            if VisualsState.chinaHatRainbow then
                h.Color = Color3.fromHSV((tick() % 5) / 5, 1, 1)
            else
                h.Color = VisualsState.chinaHatColor
            end
            h.Transparency = VisualsState.chinaHatTransparency
            local m = h:FindFirstChildOfClass("SpecialMesh")
            if m then
                m.Scale = Vector3.new(2.4, 1.6, 2.4) * VisualsState.chinaHatSize
            end
        end)
    end

    local function applyChinaHatColors()
        for _, obj in ipairs(VisualsState.chinaHatParts) do
            if obj:IsA("BasePart") then
                obj.Color = VisualsState.chinaHatColor
                obj.Transparency = VisualsState.chinaHatTransparency
            end
        end
    end
    ...
    local function destroyTrails()
        for _, t in ipairs(VisualsState.trailObjects) do
            pcall(function() if t and t.Parent then t:Destroy() end end)
        end
        VisualsState.trailObjects = {}
    end
    ...
    local function createTrails()
        ...
        local parts = {
            ...
            char:FindFirstChild("Left Arm") or char:FindFirstChild("LeftUpperArm") or char:FindFirstChild("LeftHand"),
            char:FindFirstChild("Right Arm") or char:FindFirstChild("RightUpperArm") or char:FindFirstChild("RightHand"),
            char:FindFirstChild("Left Leg") or char:FindFirstChild("LeftUpperLeg") or char:FindFirstChild("LeftFoot"),
            char:FindFirstChild("Right Leg") or char:FindFirstChild("RightUpperLeg") or char:FindFirstChild("RightFoot"),
        }
        for _, part in ipairs(parts) do
            if part and part:IsA("BasePart") then
                local a0 = Instance.new("Attachment")
                a0.Name = "_PlutonTrailA0"
                a0.Position = Vector3.new(0, 0.4, 0)
                a0.Parent = part
                local a1 = Instance.new("Attachment")
                a1.Name = "_PlutonTrailA1"
                a1.Position = Vector3.new(0, -0.4, 0)
                a1.Parent = part
                local trail = Instance.new("Trail")
                trail.Name = "_PlutonTrail"
                trail.Attachment0 = a0
                trail.Attachment1 = a1
                trail.Lifetime = VisualsState.trailLifetime
                ...
                trail.Color = ColorSequence.new({
                    ColorSequenceKeypoint.new(0, VisualsState.trailColor),
                    ColorSequenceKeypoint.new(1, VisualsState.trailFadeColor),
                })
                ...
                trail.Parent = part
                table.insert(VisualsState.trailObjects, a0)
                table.insert(VisualsState.trailObjects, a1)
                table.insert(VisualsState.trailObjects, trail)
            end
        end
    end

    local function applyTrailColors()
        for _, obj in ipairs(VisualsState.trailObjects) do
            if obj:IsA("Trail") then
                obj.Color = ColorSequence.new({
                    ColorSequenceKeypoint.new(0, VisualsState.trailColor),
                    ColorSequenceKeypoint.new(1, VisualsState.trailFadeColor),
                })
                obj.Lifetime = VisualsState.trailLifetime
            end
        end
    end
    ...
    local function restoreSkybox()
        for _, obj in ipairs(VisualsState.customSkyParts) do
            pcall(function() if obj and obj.Parent then obj:Destroy() end end)
        end
        VisualsState.customSkyParts = {}
        local existing = Lighting:FindFirstChildOfClass("Sky")
        if VisualsState.originalSky then
            if existing then existing:Destroy() end
            local clone = VisualsState.originalSky:Clone()
            clone.Parent = Lighting
            VisualsState.originalSky = nil
        end
        local cc = Lighting:FindFirstChild("_PlutonSkyCC")
        if cc then cc:Destroy() end
    end

    local function applySkybox(assetId)
        if not VisualsState.originalSky then
            local existing = Lighting:FindFirstChildOfClass("Sky")
            if existing then
                VisualsState.originalSky = existing:Clone()
            end
        end
        ...
        if not cc then
            ...
            cc.Name = "_PlutonSkyCC"
            cc.Parent = Lighting
            table.insert(VisualsState.customSkyParts, cc)
        end
        cc.TintColor = VisualsState.skyboxColor
        cc.Enabled = true
        cc.Brightness = 0
        cc.Contrast = 0.05
        cc.Saturation = 0.1
    ...
    local function saveOriginalLighting()
        if VisualsState.originalLighting then return end
        VisualsState.originalLighting = {
            Brightness = Lighting.Brightness,
    ...
    local function destroyShaders()
        for _, e in ipairs(VisualsState.shaderEffects) do
            pcall(function() if e and e.Parent then e:Destroy() end end)
        end
        VisualsState.shaderEffects = {}
        if VisualsState.originalLighting then
            local o = VisualsState.originalLighting
            pcall(function()
            ...
            VisualsState.originalLighting = nil
        end
        local atm = Lighting:FindFirstChild("_PlutonAtmosphere")
        if atm then atm:Destroy() end
    end

    local function applyShaders()
        destroyShaders()
        if not VisualsState.shaders then return end
        ...
        bloom.Name = "_PlutonBloom"
        bloom.Intensity = VisualsState.bloomIntensity
        bloom.Size = VisualsState.bloomSize
        bloom.Threshold = VisualsState.bloomThreshold
        bloom.Parent = Lighting
        table.insert(VisualsState.shaderEffects, bloom)

        local cc = Instance.new("ColorCorrectionEffect")
        cc.Name = "_PlutonCC"
        cc.Brightness = VisualsState.ccBrightness
        cc.Contrast = VisualsState.ccContrast
        cc.Saturation = VisualsState.ccSaturation
        cc.TintColor = VisualsState.ccTint
        cc.Parent = Lighting
        table.insert(VisualsState.shaderEffects, cc)

        local rays = Instance.new("SunRaysEffect")
        rays.Name = "_PlutonSunRays"
        rays.Intensity = VisualsState.sunRaysIntensity
        rays.Spread = VisualsState.sunRaysSpread
        rays.Parent = Lighting
        table.insert(VisualsState.shaderEffects, rays)

        local atm = Lighting:FindFirstChildOfClass("Atmosphere")
        if not atm then
            atm = Instance.new("Atmosphere")
            atm.Name = "_PlutonAtmosphere"
            atm.Parent = Lighting
        else
            atm = atm:Clone()
            atm.Name = "_PlutonAtmosphere"
        ...
        if atm then
            pcall(function()
                atm.Density = VisualsState.atmosphereDensity
                atm.Color = VisualsState.atmosphereColor
                atm.Decay = VisualsState.atmosphereColor
                atm.Glare = VisualsState.atmosphereGlare
                atm.Haze = VisualsState.atmosphereHaze
            end)
        end

        if VisualsState.dofEnabled then
            local dof = Instance.new("DepthOfFieldEffect")
            dof.Name = "_PlutonDOF"
            dof.FarIntensity = VisualsState.dofFarIntensity
            dof.FocusDistance = VisualsState.dofFocusDistance
            dof.InFocusRadius = 30
            dof.NearIntensity = 0.05
            dof.Parent = Lighting
            table.insert(VisualsState.shaderEffects, dof)
        end
    end

    local function refreshShaderValues()
        if not VisualsState.shaders then return end
        for _, e in ipairs(VisualsState.shaderEffects) do
            if e:IsA("BloomEffect") then
                e.Intensity = VisualsState.bloomIntensity
                e.Size = VisualsState.bloomSize
                e.Threshold = VisualsState.bloomThreshold
            elseif e:IsA("ColorCorrectionEffect") and e.Name == "_PlutonCC" then
                e.Brightness = VisualsState.ccBrightness
                e.Contrast = VisualsState.ccContrast
                e.Saturation = VisualsState.ccSaturation
                e.TintColor = VisualsState.ccTint
            elseif e:IsA("SunRaysEffect") then
                e.Intensity = VisualsState.sunRaysIntensity
                e.Spread = VisualsState.sunRaysSpread
            elseif e:IsA("DepthOfFieldEffect") then
                e.FarIntensity = VisualsState.dofFarIntensity
                e.FocusDistance = VisualsState.dofFocusDistance
            end
        end
        local atm = Lighting:FindFirstChildOfClass("Atmosphere")
        if atm then
    ...
    local function destroySnow()
        if VisualsState.snowConn then
            VisualsState.snowConn:Disconnect()
            VisualsState.snowConn = nil
        end
        for _, p in ipairs(VisualsState.snowParts) do
            pcall(function() if p and p.Parent then p:Destroy() end end)
        end
        VisualsState.snowParts = {}
        local folder = workspace:FindFirstChild("_PlutonSnow")
    ...
    local function createSnow()
        ...
        folder.Name = "_PlutonSnow"
        folder.Parent = workspace

        local flakes = {}
        local amount = math.clamp(math.floor(VisualsState.snowAmount), 10, 120)
        for i = 1, amount do
            ...
            flake.Color = VisualsState.snowColor
            flake.Transparency = 0.15
            local sz = 0.12 + math.random() * 0.18
            flake.Size = Vector3.new(sz, sz * 0.15, sz)
            flake.Parent = folder
            table.insert(VisualsState.snowParts, flake)
            flakes[i] = {
                part = flake,
                x = 0, y = 0, z = 0,
                rot = math.random() * math.pi * 2,
                rotSpeed = (math.random() - 0.5) * 2,
                fall = 8 + math.random() * 14,
                sway = 1 + math.random() * 2,
                phase = math.random() * math.pi * 2,
            }
        end

        local function respawnFlake(f, camPos)
            f.x = camPos.X + (math.random() - 0.5) * 120
            f.y = camPos.Y + 40 + math.random() * 30
            f.z = camPos.Z + (math.random() - 0.5) * 120
            f.part.Color = VisualsState.snowColor
        end

        local cam = workspace.CurrentCamera
        local camPos = cam and cam.CFrame.Position or Vector3.zero
        for _, f in ipairs(flakes) do
            respawnFlake(f, camPos)
            f.part.CFrame = CFrame.new(f.x, f.y, f.z)
        end

        VisualsState.snowConn = RunService.RenderStepped:Connect(function(dt)
            if not VisualsState.snowEnabled then return end
            cam = workspace.CurrentCamera
            if not cam then return end
            camPos = cam.CFrame.Position
            local speed = VisualsState.snowSpeed
            for _, f in ipairs(flakes) do
                f.y = f.y - f.fall * speed * dt
                f.x = f.x + math.sin(tick() * f.sway + f.phase) * 0.8 * dt
                f.z = f.z + math.cos(tick() * f.sway * 0.7 + f.phase) * 0.8 * dt
                f.rot = f.rot + f.rotSpeed * dt
                if f.y < camPos.Y - 15 then
                    respawnFlake(f, camPos)
                end
                
                if math.abs(f.x - camPos.X) > 80 or math.abs(f.z - camPos.Z) > 80 then
                    respawnFlake(f, camPos)
                end
                f.part.CFrame = CFrame.new(f.x, f.y, f.z) * CFrame.Angles(f.rot, f.rot * 0.5, f.rot * 0.3)
            end
        end)
    end
    ...
    LocalPlayer.CharacterAdded:Connect(function()
        task.wait(0.6)
        if VisualsState.chinaHat then createChinaHat() end
        if VisualsState.trails then createTrails() end
    end)


    
    local jumpCircleParts = {}
    local jumpCircleConn = nil
    local jumpStateConn = nil

    local function cleanupJumpCircles()
        for _, p in ipairs(jumpCircleParts) do
            pcall(function() p:Destroy() end)
        end
        jumpCircleParts = {}
        if jumpCircleConn then
            pcall(function() jumpCircleConn:Disconnect() end)
            jumpCircleConn = nil
        end
        if jumpStateConn then
            pcall(function() jumpStateConn:Disconnect() end)
            jumpStateConn = nil
        end
    end

            local function spawnJumpCircle(pos)
        if not VisualsState.jumpCircles then return end
        local size = VisualsState.jumpCircleSize or 6
        local lifetime = VisualsState.jumpCircleDuration or 0.8
        local col = VisualsState.jumpCircleColor or Color3.fromRGB(0, 200, 255)
        if VisualsState.jumpCircleRainbow then
        ...
        local startSize = size * 0.3
        local endSize = size
        local thickness = 0.18

        local ring = Instance.new("Part")
        ring.Name = "_PlutonJumpCircle"
        ring.Anchored = true
        ...
        table.insert(jumpCircleParts, ring)

        local startT = tick()
        local conn
        conn = RunService.Heartbeat:Connect(function()
            local t = (tick() - startT) / lifetime
            ...
            local s = startSize + (endSize - startSize) * ease
            
            ring.Size = Vector3.new(thickness, s, s)
            ring.Transparency = 0.1 + t * 0.9
            if VisualsState.jumpCircleRainbow then
                ring.Color = Color3.fromHSV(((tick() * 0.5) % 5) / 5, 1, 1)
            end
        end)
    end
...
local function startJumpCircles()
        ...
        local function hookChar(char)
            ...
            jumpStateConn = hum.StateChanged:Connect(function(_, new)
                if not VisualsState.jumpCircles then return end
                if new == Enum.HumanoidStateType.Jumping then
                    local origin = root.Position - Vector3.new(0, root.Size.Y * 0.5 + 1.5, 0)
                    
                    local ray = Workspace:Raycast(root.Position, Vector3.new(0, -8, 0))
                    if ray then origin = ray.Position + Vector3.new(0, 0.05, 0) end
                    spawnJumpCircle(origin)
                end
            end)
        end
        if LocalPlayer.Character then hookChar(LocalPlayer.Character) end
        jumpCircleConn = LocalPlayer.CharacterAdded:Connect(hookChar)
    end
    ...
    local chinaSec = VisualsTab:AddLeftGroupbox("China Hat")
    chinaSec:AddToggle("ChinaHatEnabled", {
        Text = "China Hat",
        Default = false,
        Callback = function(v)
            VisualsState.chinaHat = v
            if v then createChinaHat() else destroyChinaHat() end
        end,
    })
    chinaSec:AddLabel("Hat Color"):AddColorPicker("ChinaHatColor", {
        Default = VisualsState.chinaHatColor,
        Title = "China Hat Color",
        Callback = function(col)
            VisualsState.chinaHatColor = col
            applyChinaHatColors()
        end,
    })
    chinaSec:AddToggle("ChinaHatRainbow", {
        Text = "Rainbow Hat",
        Default = false,
        Callback = function(v)
            VisualsState.chinaHatRainbow = v
        end,
    })
    chinaSec:AddLabel("Accent Color"):AddColorPicker("ChinaHatAccent", {
        Default = VisualsState.chinaHatAccent,
        Title = "Hat Accent",
        Callback = function(col)
            VisualsState.chinaHatAccent = col
            applyChinaHatColors()
        end,
    })
    chinaSec:AddSlider("ChinaHatTransparency", {
        Text = "Transparency",
        Default = 25,
        Min = 0,
        Max = 90,
        ...
        Suffix = "%",
        Callback = function(v)
            VisualsState.chinaHatTransparency = v / 100
            applyChinaHatColors()
        end,
    })
    chinaSec:AddSlider("ChinaHatSize", {
        Text = "Size",
        Default = 100,
        Min = 50,
        Max = 200,
        ...
        Suffix = "%",
        Callback = function(v)
            VisualsState.chinaHatSize = v / 100
            if VisualsState.chinaHat then createChinaHat() end
        end,
    })

    local trailSec = VisualsTab:AddLeftGroupbox("Trails")
    trailSec:AddToggle("TrailsEnabled", {
        Text = "Trails",
        Default = false,
        Callback = function(v)
            VisualsState.trails = v
            if v then createTrails() else destroyTrails() end
        end,
    })
    trailSec:AddLabel("Trail Color"):AddColorPicker("TrailColor", {
        Default = VisualsState.trailColor,
        Title = "Trail Start Color",
        Callback = function(col)
            VisualsState.trailColor = col
            applyTrailColors()
        end,
    })
    trailSec:AddLabel("Trail Fade Color"):AddColorPicker("TrailFadeColor", {
        Default = VisualsState.trailFadeColor,
        Title = "Trail End Color",
        Callback = function(col)
            VisualsState.trailFadeColor = col
            applyTrailColors()
        end,
    })
    trailSec:AddSlider("TrailLifetime", {
        Text = "Lifetime Length",
        Default = 50,
        Min = 10,
        Max = 200,
        Rounding = 0,
        Suffix = " x0.01s",
        Callback = function(v)
            VisualsState.trailLifetime = v / 100
            applyTrailColors()
        end,
    })


    local jumpSec = VisualsTab:AddLeftGroupbox("Jump Circles")
    jumpSec:AddToggle("JumpCirclesEnabled", {
        Text = "Jump Circles",
        Default = false,
        Callback = function(v)
            VisualsState.jumpCircles = v
            if v then startJumpCircles() else stopJumpCircles() end
        end,
    })
    jumpSec:AddLabel("Circle Color"):AddColorPicker("JumpCircleColor", {
        Default = Color3.fromRGB(0, 200, 255),
        Title = "Jump Circle Color",
        Callback = function(col)
            VisualsState.jumpCircleColor = col
        end,
    })
    jumpSec:AddToggle("JumpCircleRainbow", {
        Text = "Rainbow",
        Default = false,
        Callback = function(v)
            VisualsState.jumpCircleRainbow = v
        end,
    })
    jumpSec:AddSlider("JumpCircleSize", {
        Text = "Size",
        Default = 6,
        Min = 2,
        Max = 20,
        Rounding = 0,
        Callback = function(v)
            VisualsState.jumpCircleSize = v
        end,
    })
    jumpSec:AddSlider("JumpCircleLifetime", {
        Text = "Lifetime",
        Default = 8,
        Min = 2,
        Max = 20,
        Rounding = 0,
        Callback = function(v)
            VisualsState.jumpCircleLifetime = v / 10
        end,
    })

    local snowSec = VisualsTab:AddLeftGroupbox("Snowflakes")
    snowSec:AddToggle("SnowEnabled", {
        Text = "Falling Snowflakes",
        Default = false,
        Callback = function(v)
            VisualsState.snowEnabled = v
            if v then createSnow() else destroySnow() end
        end,
    })
    snowSec:AddLabel("Snow Color"):AddColorPicker("SnowColor", {
        Default = VisualsState.snowColor,
        Title = "Snowflake Color",
        Callback = function(col)
            VisualsState.snowColor = col
            for _, p in ipairs(VisualsState.snowParts) do
                if p:IsA("BasePart") then p.Color = col end
            end
        end,
    })
    snowSec:AddSlider("SnowAmount", {
        Text = "Amount",
        Default = 40,
        Min = 10,
        Max = 120,
        Rounding = 0,
        Callback = function(v)
            VisualsState.snowAmount = v
            if VisualsState.snowEnabled then createSnow() end
        end,
    })
    snowSec:AddSlider("SnowSpeed", {
        Text = "Fall Speed",
        Default = 100,
        Min = 20,
        Max = 300,
        ...
        Suffix = "%",
        Callback = function(v)
            VisualsState.snowSpeed = v / 100
        end,
    })

    local skySec = VisualsTab:AddRightGroupbox("Skybox")
    skySec:AddToggle("SkyboxEnabled", {
        Text = "Custom Skybox",
        Default = false,
        Callback = function(v)
            VisualsState.skybox = v
            if v then
                applySkybox(VisualsState.skyboxId)
            else
                restoreSkybox()
            end
        end,
    ...
    skySec:AddDropdown("SkyboxPreset", {
        Text = "Preset",
        Values = presetNames,
        Default = 1,
        Callback = function(name)
            local id = SKYBOX_PRESETS[name]
            if name == "Default (restore)" or id == "" then
                if VisualsState.skybox then
                    restoreSkybox()
                    VisualsState.skybox = false
                    if Toggles.SkyboxEnabled then
                        pcall(function() Toggles.SkyboxEnabled:SetValue(false) end)
                    end
                end
            else
                VisualsState.skyboxId = id
                if VisualsState.skybox then
                    applySkybox(id)
                end
            end
    ...
    skySec:AddInput("SkyboxCustomId", {
        Text = "Custom Asset ID",
        Default = "",
        Placeholder = "rbxassetid://123456",
        Callback = function(val)
            if val and val ~= "" then
                if not string.find(val, "rbxassetid://") then
                    val = "rbxassetid://" .. val:gsub("%D", "")
                end
                VisualsState.skyboxId = val
                if VisualsState.skybox then
                    applySkybox(val)
                end
            end
    ...
    skySec:AddLabel("Sky Tint"):AddColorPicker("SkyboxTint", {
        Default = VisualsState.skyboxColor,
        Title = "Sky Tint Color",
        Callback = function(col)
            VisualsState.skyboxColor = col
            if VisualsState.skybox then
                local cc = Lighting:FindFirstChild("_PlutonSkyCC")
                if cc then cc.TintColor = col end
            end
        end,
    })

    local shaderSec = VisualsTab:AddRightGroupbox("Shaders")
    shaderSec:AddToggle("ShadersEnabled", {
        Text = "Enable Shaders",
        Default = false,
        Callback = function(v)
            VisualsState.shaders = v
            if v then applyShaders() else destroyShaders() end
        end,
    })
    shaderSec:AddLabel("Tint"):AddColorPicker("ShaderTint", {
        Default = VisualsState.ccTint,
        Title = "Shader Tint",
        Callback = function(col)
            VisualsState.ccTint = col
            refreshShaderValues()
        end,
    })
    shaderSec:AddSlider("BloomIntensity", {
        Text = "Bloom",
        Default = 120,
        Min = 0,
        Max = 300,
        ...
        Suffix = "%",
        Callback = function(v)
            VisualsState.bloomIntensity = v / 100
            refreshShaderValues()
        end,
    })
    shaderSec:AddSlider("BloomSize", {
        Text = "Bloom Size",
        Default = 24,
        Min = 4,
        Max = 56,
        Rounding = 0,
        Callback = function(v)
            VisualsState.bloomSize = v
            refreshShaderValues()
        end,
    })
    shaderSec:AddSlider("Contrast", {
        Text = "Contrast",
        Default = 15,
        Min = -50,
        Max = 50,
        Rounding = 0,
        Callback = function(v)
            VisualsState.ccContrast = v / 100
            refreshShaderValues()
        end,
    })
    shaderSec:AddSlider("Saturation", {
        Text = "Saturation",
        Default = 20,
        Min = -50,
        Max = 100,
        Rounding = 0,
        Callback = function(v)
            VisualsState.ccSaturation = v / 100
            refreshShaderValues()
        end,
    })
    shaderSec:AddSlider("Brightness", {
        Text = "Brightness",
        Default = 5,
        Min = -30,
        Max = 30,
        Rounding = 0,
        Callback = function(v)
            VisualsState.ccBrightness = v / 100
            refreshShaderValues()
        end,
    })
    shaderSec:AddSlider("SunRays", {
        Text = "Sun Rays",
        Default = 12,
        Min = 0,
        Max = 50,
        ...
        Suffix = "%",
        Callback = function(v)
            VisualsState.sunRaysIntensity = v / 100
            refreshShaderValues()
        end,
    })
    shaderSec:AddSlider("Atmosphere", {
        Text = "Atmosphere",
        Default = 30,
        Min = 0,
        Max = 100,
        ...
        Suffix = "%",
        Callback = function(v)
            VisualsState.atmosphereDensity = v / 100
            refreshShaderValues()
        end,
    })
    shaderSec:AddLabel("Atmosphere Color"):AddColorPicker("AtmoColor", {
        Default = VisualsState.atmosphereColor,
        Title = "Atmosphere Color",
        Callback = function(col)
            VisualsState.atmosphereColor = col
            refreshShaderValues()
        end,
    })
    shaderSec:AddToggle("DOFEnabled", {
        Text = "Depth of Field",
        Default = false,
        Callback = function(v)
            VisualsState.dofEnabled = v
            if VisualsState.shaders then applyShaders() end
        end,
    })
    shaderSec:AddSlider("DOFIntensity", {
        Text = "DOF Intensity",
        Default = 15,
        Min = 0,
        Max = 50,
        Rounding = 0,
        Callback = function(v)
            VisualsState.dofFarIntensity = v / 100
            refreshShaderValues()
        end,
    })
...
do
    local pcldSec = VisualsTab:AddLeftGroupbox("PCLD Customizer")
    local cfg = Settings.Misc.pcldCustom or {}
    cfg.enabled = cfg.enabled or false
    cfg.rainbow = cfg.rainbow or false
    cfg.pulse = cfg.pulse ~= false
    cfg.color = cfg.color or Color3.fromRGB(0, 220, 255)
    cfg.color2 = cfg.color2 or Color3.fromRGB(180, 80, 255)
    cfg.outlineColor = cfg.outlineColor or Color3.fromRGB(255, 255, 255)
    cfg.outlineThickness = cfg.outlineThickness or 0.15
    cfg.outlineEnabled = cfg.outlineEnabled ~= false
    cfg.transparency = cfg.transparency or 0.35
    cfg.boxESP = cfg.boxESP ~= false
    cfg.glow = cfg.glow ~= false
    cfg.pulseSpeed = cfg.pulseSpeed or 2
    cfg.smooth = cfg.smooth ~= false
    cfg.smoothSpeed = cfg.smoothSpeed or 12
    Settings.Misc.pcldCustom = cfg
    ...
    local adornFolder = CoreGui:FindFirstChild("_PlutonPCLDAdorn")
    if not adornFolder then
        adornFolder = Instance.new("Folder")
        adornFolder.Name = "_PlutonPCLDAdorn"
        adornFolder.Parent = CoreGui
    end

    local function isPCLD(obj)
        return obj and obj:IsA("BasePart") and (
            obj.Name == "PlayerCharacterLocationDetector"
            or string.lower(obj.Name) == "partesp"
        )
    end

    local function clearOne(part, restorePart)
        local d = boxes[part]
        if not d then return end
        pcall(function() if d.box then d.box:Destroy() end end)
        pcall(function() if d.highlight then d.highlight:Destroy() end end)
        pcall(function() if d.light then d.light:Destroy() end end)
        pcall(function() if d.outline then d.outline:Destroy() end end)
        pcall(function() if d.smoothPart then d.smoothPart:Destroy() end end)
        pcall(function() if d.smoothBox then d.smoothBox:Destroy() end end)
        boxes[part] = nil
        if restorePart and part and part.Parent then
            pcall(function()
                part.Transparency = 1
    ...
    local function clearAll(fullStop)
        for part in pairs(boxes) do clearOne(part, true) end
        boxes = {}
        if fullStop then
    ...
    local function adorn(part)
        if boxes[part] or not part or not part.Parent then return end
        local pos = part.Position
        if pos.X == 0 and pos.Y == 0 and pos.Z == 0 then return end

        local data = { smoothCF = part.CFrame }
        
        local useSmooth = cfg.smooth == true
        local adornTarget = part

        if useSmooth then
            local sp = Instance.new("Part")
            sp.Name = "_PlutonPCLDSmooth"
            sp.Anchored = true
            sp.CanCollide = false
            sp.CanQuery = false
            sp.CanTouch = false
            ...
            sp.Transparency = 1
            sp.Size = part.Size
            sp.CFrame = part.CFrame
            sp.Parent = adornFolder
            data.smoothPart = sp
            adornTarget = sp
        end
        ...
        if cfg.boxESP then
            ...
            box.Name = "_PlutonPCLDBox"
            box.Adornee = adornTarget
            box.AlwaysOnTop = true
            box.ZIndex = 8
            box.Color3 = cfg.color
            box.Transparency = cfg.transparency
        ...
        if cfg.glow then
            local hl = Instance.new("Highlight")
            hl.Name = "_PlutonPCLDHL"
            hl.Adornee = adornTarget
            hl.FillColor = cfg.color
            hl.OutlineColor = cfg.outlineColor or cfg.color2
            hl.FillTransparency = 0.55
            hl.OutlineTransparency = cfg.outlineEnabled and 0.1 or 1
            hl.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
            hl.Parent = adornFolder
            data.highlight = hl
            local pl = Instance.new("PointLight")
            pl.Name = "_PlutonPCLDLight"
            pl.Color = cfg.color
            pl.Brightness = 1.2
            pl.Range = 12
            pl.Shadows = false
            pl.Parent = adornTarget
            data.light = pl
        end
        if cfg.outlineEnabled then
            local sb = Instance.new("SelectionBox")
            sb.Name = "_PlutonPCLDOutline"
            sb.Adornee = adornTarget
            sb.LineThickness = cfg.outlineThickness or 0.15
            sb.Color3 = cfg.outlineColor or Color3.new(1, 1, 1)
            sb.SurfaceColor3 = cfg.outlineColor or Color3.new(1, 1, 1)
            sb.SurfaceTransparency = 0.85
            sb.Transparency = 0
            sb.Parent = adornFolder
            data.outline = sb
        end
        ...
        pcall(function()
            ...
            part.Transparency = math.clamp(cfg.transparency * 0.85, 0.35, 0.9)
        end)
        boxes[part] = data
    end
    ...
    local function scan()
        for _, obj in ipairs(workspace:GetDescendants()) do
            if isPCLD(obj) then adorn(obj) end
        end
    end
    ...
    local function startAnim()
        ...
        animConn = RunService.RenderStepped:Connect(function(dt)
            ...
            tAcc = tAcc + dt * (cfg.pulseSpeed or 2)
            local pulse = cfg.pulse and (0.5 + 0.5 * math.sin(tAcc * 2.2)) or 1
            local col = cfg.color
            if cfg.rainbow then
                col = Color3.fromHSV((tick() * 0.25) % 1, 0.95, 1)
            elseif cfg.pulse then
                
                local a = 0.5 + 0.5 * math.sin(tAcc)
                col = cfg.color:Lerp(cfg.color2, a)
            end
            local trans = math.clamp(cfg.transparency * (1.15 - pulse * 0.35), 0.05, 0.85)
            for part, data in pairs(boxes) do
                if not part or not part.Parent then
                    clearOne(part)
                else
                    local pos = part.Position
                    ...
                    if pos.X == 0 and pos.Y == 0 and pos.Z == 0 then
                        if data.box then data.box.Transparency = 1 end
                        if data.highlight then data.highlight.Enabled = false end
                        if data.light then data.light.Enabled = false end
                    else
                        if data.light then data.light.Enabled = true end
                        pcall(function()
                            part.Color = col
                            part.Transparency = trans * 0.7
                            part.Material = Enum.Material.Neon
                            if data.box then
                                data.box.Color3 = col
                                data.box.Transparency = trans
                                local s = 0.08 + 0.12 * pulse
                                data.box.Size = part.Size + Vector3.new(s, s, s)
                            end
                            if data.highlight then
                                data.highlight.Enabled = true
                                data.highlight.FillColor = col
                                data.highlight.OutlineColor = cfg.outlineColor or cfg.color2
                                data.highlight.OutlineTransparency = cfg.outlineEnabled and 0.05 or 1
                                data.highlight.FillTransparency = 0.45 + (1 - pulse) * 0.25
                            end
                            if data.outline then
                                data.outline.Visible = cfg.outlineEnabled ~= false
                                data.outline.Color3 = cfg.outlineColor or Color3.new(1, 1, 1)
                                data.outline.SurfaceColor3 = cfg.outlineColor or Color3.new(1, 1, 1)
                                data.outline.LineThickness = (cfg.outlineThickness or 0.15) * (0.85 + 0.3 * pulse)
                            end
                            if data.light then
                                data.light.Color = col
                                data.light.Brightness = 0.6 + pulse * 1.2
                            end
                            
                            if data.smoothPart and data.smoothPart.Parent then
                                local speed = cfg.smoothSpeed or 12
                                local alpha = math.clamp(dt * speed, 0, 1)
                                data.smoothCF = data.smoothCF and data.smoothCF:Lerp(part.CFrame, alpha) or part.CFrame
                                data.smoothPart.CFrame = data.smoothCF
                                data.smoothPart.Size = part.Size
                                if data.box then
                                    local s = 0.08 + 0.12 * pulse
                                    data.box.Size = part.Size + Vector3.new(s, s, s)
                                end
    ...
    local function setEnabled(on)
        ...
        if on then
            ...
            scanConn = workspace.DescendantAdded:Connect(function(obj)
                if cfg.enabled and isPCLD(obj) then
                    task.defer(function() adorn(obj) end)
                end
            end)
            startAnim()
    ...
    _G.PlutonPCLDSetEnabled = setEnabled

    pcldSec:AddToggle("PCLDCustomEnabled", {
        Text = "Enable PCLD",
        Default = false,
        Callback = function(v) setEnabled(v) end,
    })
    pcldSec:AddToggle("PCLDRainbow", {
        Text = "Rainbow",
        Default = false,
        Callback = function(v) cfg.rainbow = v end,
    })
    pcldSec:AddToggle("PCLDPulse", {
        Text = "Smooth Pulse",
        Default = true,
        Callback = function(v) cfg.pulse = v end,
    })
    pcldSec:AddToggle("PCLDBoxESP", {
        Text = "Box ESP",
        Default = true,
        Callback = function(v)
            cfg.boxESP = v
            if cfg.enabled then clearAll(); setEnabled(true) end
        end,
    })
    pcldSec:AddToggle("PCLDGlow", {
        Text = "Glow / Highlight",
        Default = true,
        Callback = function(v)
            cfg.glow = v
    ...
    pcldSec:AddLabel("Primary Color"):AddColorPicker("PCLDColor1", {
        Default = cfg.color,
        Title = "PCLD Primary",
        Callback = function(c) cfg.color = c end,
    })
    pcldSec:AddLabel("Secondary Color"):AddColorPicker("PCLDColor2", {
        Default = cfg.color2,
        Title = "PCLD Secondary",
        Callback = function(c) cfg.color2 = c end,
    })
    pcldSec:AddSlider("PCLDTransparency", {
        Text = "Transparency",
        Default = 35, Min = 5, Max = 90, Rounding = 0, Suffix = "%",
        Callback = function(v) cfg.transparency = v / 100 end,
    })
    pcldSec:AddSlider("PCLDPulseSpeed", {
        Text = "Pulse Speed",
        Default = 20, Min = 5, Max = 60, Rounding = 0,
        Callback = function(v) cfg.pulseSpeed = v / 10 end,
    })
    pcldSec:AddToggle("PCLDOutline", {
        Text = "Outline",
        Default = true,
        Callback = function(v)
            cfg.outlineEnabled = v
            if cfg.enabled then clearAll(); setEnabled(true) end
        end,
    })
    pcldSec:AddLabel("Outline Color"):AddColorPicker("PCLDOutlineColor", {
        Default = Color3.fromRGB(255, 255, 255),
        Title = "PCLD Outline",
        Callback = function(c) cfg.outlineColor = c end,
    })
    pcldSec:AddSlider("PCLDOutlineThick", {
        Text = "Outline Thickness",
        Default = 15, Min = 5, Max = 40, Rounding = 0,
        Callback = function(v) cfg.outlineThickness = v / 100 end,
    })
    pcldSec:AddToggle("PCLDSmooth", {
        Text = "Smooth Follow",
        Default = true,
        Callback = function(v)
            cfg.smooth = v
    ...
    pcldSec:AddSlider("PCLDSmoothSpeed", {
        Text = "Smooth Speed",
        Default = 12, Min = 2, Max = 40, Rounding = 0,
        Callback = function(v) cfg.smoothSpeed = v end,
    })
end


    local bhSec = VisualsTab:AddRightGroupbox("Black Hole")
    local BHK = {
        ColorMode = "Default",
        NeonGlow = false,
        Silent = false,
    ...
    local function BHK_ApplyToModel(model)
        ...
        hole.Material = BHK.NeonGlow and Enum.Material.Neon or Enum.Material.Plastic
        if not BHK.RainbowActive then
            local ct = BHK_Colors[BHK.ColorMode]
            if ct then
                hole.Color = ct.hole
                local att = hole:FindFirstChild("Attachment")
                local beam = att and att:FindFirstChild("Beam")
        ...
        local beam = att and att:FindFirstChild("Beam")
        if beam then
            beam.Width0 = BHK.BeamWidth0
            beam.Width1 = BHK.BeamWidth1
            beam.Transparency = NumberSequence.new(BHK.BeamTransparency / 100)
        end
        local gui = hole:FindFirstChild("BillboardGui")
        if gui then
            gui.Size = UDim2.new(BHK.BillboardSize, 0, BHK.BillboardSize, 0)
            gui.Enabled = not BHK.HideBillboard
        end
        local drone = hole:FindFirstChild("Drone")
        local scream = hole:FindFirstChild("Scream")
        if drone then drone.Volume = BHK.Silent and 0 or 1 end
        if scream then
            scream.Volume = BHK.Silent and 0 or 1
            local reverb = scream:FindFirstChildOfClass("ReverbSoundEffect")
            if reverb then reverb.Enabled = BHK.ReverbEnabled end
        end
    end
    ...
    local function BHK_SetupWatcher()
        if BHK.WatcherConn then BHK.WatcherConn:Disconnect() end
        BHK.WatcherConn = workspace.ChildAdded:Connect(function(child)
            if child.Name == "BlackHoleKick" then
                task.wait(0.1)
                BHK_ApplyToModel(child)
                if BHK.Realistic then
                    task.spawn(function()
                        local ok, realisticModel = pcall(function()
                            return game:GetObjects("rbxassetid://16797584940")[1]
                        end)
                        if ok and realisticModel then
                            local hole = child:FindFirstChild("Hole")
                            if hole then
    ...
    bhSec:AddToggle("BHKWhiteHole", {
        Text = "White Hole Mode",
        Default = false,
        Callback = function(v)
            _G.WhiteHoleEnabled = v
            if v then
                BHK.ColorMode = "White Hole"
            else
                BHK.ColorMode = "Default"
            end
            BHK_ApplyCurrent()
        end,
    })
    bhSec:AddToggle("BHKRealistic", {
            ...
            if v then
                ...
                if current then
                    task.spawn(function()
                        ...
                        if ok and realisticModel then
                            local hole = current:FindFirstChild("Hole")
                            if hole then
                                for _, obj in pairs(realisticModel:GetDescendants()) do
                                    if obj:IsA("ParticleEmitter") or obj:IsA("Beam") or obj:IsA("Sound") or obj:IsA("Trail") then
    ...
    bhSec:AddDropdown("BHKColorMode", {
        Text = "Hole Color Mode",
        Values = {"Default", "White Hole", "Red Hole", "Blue Hole", "Green Hole", "Gold Hole", "Cyan Hole", "Pink Hole"},
        Default = 1,
        Callback = function(v)
            BHK.ColorMode = v
            BHK.RainbowActive = false
            if BHK.RainbowConn then BHK.RainbowConn:Disconnect() BHK.RainbowConn = nil end
            BHK_ApplyCurrent()
        end,
    })
    bhSec:AddToggle("BHKNeon", {
        Text = "Neon Glow",
        Default = false,
        Callback = function(v)
            BHK.NeonGlow = v
    ...
    bhSec:AddToggle("BHKRainbow", {
        ...
        Callback = function(v)
            ...
            if v then
                local hue = 0
                BHK.RainbowConn = RunService.Heartbeat:Connect(function(dt)
                    hue = (hue + dt * 0.3) % 1
    ...
    bhSec:AddSlider("BHKBeamW0", {
        Text = "Beam Width (Inner)",
        Default = 1, Min = 0, Max = 20, Rounding = 1,
        Callback = function(v)
            BHK.BeamWidth0 = v
            local model = workspace:FindFirstChild("BlackHoleKick")
            local hole = model and model:FindFirstChild("Hole")
            local att = hole and hole:FindFirstChild("Attachment")
            local beam = att and att:FindFirstChild("Beam")
            if beam then beam.Width0 = v end
        end,
    })
    bhSec:AddSlider("BHKBeamW1", {
        Text = "Beam Width (Outer)",
        Default = 1, Min = 0, Max = 20, Rounding = 1,
        Callback = function(v)
            BHK.BeamWidth1 = v
            local model = workspace:FindFirstChild("BlackHoleKick")
    ...
    bhSec:AddSlider("BHKBeamTransp", {
        Text = "Beam Transparency",
        Default = 0, Min = 0, Max = 100, Rounding = 0, Suffix = "%",
        Callback = function(v)
            BHK.BeamTransparency = v
            local model = workspace:FindFirstChild("BlackHoleKick")
            local hole = model and model:FindFirstChild("Hole")
            local att = hole and hole:FindFirstChild("Attachment")
            local beam = att and att:FindFirstChild("Beam")
    ...
    bhSec:AddSlider("BHKBillboardSize", {
        Text = "Billboard Size",
        Default = 10, Min = 2, Max = 40, Rounding = 0,
        Callback = function(v)
            BHK.BillboardSize = v
            local model = workspace:FindFirstChild("BlackHoleKick")
            local hole = model and model:FindFirstChild("Hole")
    ...
    bhSec:AddToggle("BHKHideBillboard", {
        Text = "Hide Billboard (Stealth)",
        Default = false,
        Callback = function(v)
            BHK.HideBillboard = v
            local model = workspace:FindFirstChild("BlackHoleKick")
    ...
    bhSec:AddToggle("BHKSilent", {
        Text = "Silent Black Hole",
        Default = false,
        Callback = function(v)
            BHK.Silent = v
            local model = workspace:FindFirstChild("BlackHoleKick")
    ...
    bhSec:AddToggle("BHKReverb", {
        Text = "Scream Reverb Effect",
        Default = true,
        Callback = function(v)
            BHK.ReverbEnabled = v
            local model = workspace:FindFirstChild("BlackHoleKick")
...
SaveManager:SetIgnoreIndexes({
    ...
    "OwnershipKickTarget",
    "BlobTargetDropdown",
    "PalletFlingTargetDropdown",
    "CloneFlingTargetDropdown",
    "GlassBoxFlingTargetDropdown",
    "SelfFlingTargetDropdown",
    "MiscBringDropdown",
    "SnowballTargetDropdown",
})
ThemeManager:SetFolder("PlutonV2")
SaveManager:SetFolder("PlutonV2")
...
do
    local _rawNotify = Library.Notify
    if type(_rawNotify) == "function" then
        ...
        function Library.Notify(...)
            ...
            if n == 1 then
                args = { a1 }
            elseif n >= 2 and type(a1) == "table" and a1 == Library then
                
                if type(a2) == "string" then
                    args = { Library, { Title = "Pluton V2", Description = a2, Content = a2, Time = select(3, ...) or 3 } }
                else
                    args = { Library, a2 }
                end
            elseif type(a1) == "string" then
                args = { { Title = "Pluton V2", Description = a1, Content = a1, Time = (type(a2) == "number" and a2) or 3 } }
            else
                args = { a1 }
            end
            local now = tick()
            if now - lastNotifyAt < 0.15 then
                return 
            end
            lastNotifyAt = now
            task.defer(function()
                pcall(function()
                    _rawNotify(table.unpack(args))
                end)
            end)
        end
    end
...
warn("Pluton V2 (Obsidian UI) loaded successfully!")
